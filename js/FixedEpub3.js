...
function pop(self) {
  ...
  $('#modal-name').text($(self).attr('title'));
}
//表紙画像選択
...
function addmenu() {
  ...
  cmenu.childNodes[3].childNodes[3].value = '';
...
async function rewriteOPF() {
  ...
  var kindlesize = standardOPFxml.querySelector("meta[name=original-resolution]");
  var resolution = $("#imgwidth").val() + "x" + $("#imgheight").val()
  kindlesize.setAttribute("content", resolution)
  //タイトル
  ...
  //言語
  var language = standardOPFxml.getElementById("language").textContent;
  language = document.getElementById("LangSelect").value
  standardOPFxml.getElementById("language").textContent = language
  //ファイルID、uuid
  ...
  // description
  let description = standardOPFxml.getElementById("description");
        ...
        if (item.types.includes("text/plain")) {
          item.getType("text/plain").then(function (blob) {
            blob.text().then(function (text) {
              ...
              const part2 = text.match(/^\s*\[\s*([^\]]*?)\s*\]\s*(.*?)\s*$/);
              if (part) {
                $("#title").val(part[3]);
                $("#author1").val(part[1]);
                $("#author2").val(part[2]);
